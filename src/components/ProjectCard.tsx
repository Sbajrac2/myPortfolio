...
//     >
//       {/* Comic chapter label */}
//       <div className={`absolute ${isEven ? '-top-1 -left-1' : '-top-1 -right-0'} bg-manga-secondary text-white px-3 py-1 transform ${isEven ? 'rotate-3' : '-rotate-3'} shadow-md`}>
  ...
  return (
    <motion.div
      ...
      className={`manga-panel ${
        isEven ? "" : "manga-panel-alt"
      } p-6 pt-10 relative overflow-visible hover:scale-[1.02] transition-transform cursor-pointer`}
    >
      <Link to={`/projects/${slug}`} className="block">
        <div
          className={`absolute ${
            isEven ? "-top-1 -left-3" : "-top-1 -right-0"
          } bg-manga-secondary text-white px-3 py-1 transform ${
        ...
        <div className="mt-4">
          ...
          <p className="mt-4 text-foreground/90 leading-relaxed line-clamp-4">
            {description}
          </p>
      ...
      <div className="mt-6 flex items-center space-x-4">
        ...
        {demoUrl && demoUrl !== "PASTE_DEMO_LINK_HERE" && (
          <a
            href={demoUrl}
            target="_blank"
            rel="noopener noreferrer"
